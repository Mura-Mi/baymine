...
logger = Logger.new("log/old-collect-#{Date.today.strftime('%Y-%m-%d')}.log")
...
begin
  ...
  min_id = col.find.sort({id: 1}).limit(1).first[:id]

  tw.search("ベイスターズ", {max_id: min_id}).take(500).each do |tweet|
